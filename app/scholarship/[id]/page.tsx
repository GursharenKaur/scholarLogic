...
export default async function ScholarshipDetail({ params }: { params: Promise<{ id: string }> }) {
  // 1. Await the params to unlock the ID from the URL (Next.js 15+ requirement)
  ...
  if (!scholarship) {
    return (
      <div className="p-10 text-center">
        ...
        <p className="text-slate-500 mb-4">The ID {id} does not exist in the database.</p>
      ...
      <div className="max-w-3xl w-full bg-white rounded-xl shadow-sm border p-8">
        ...
        <div className="grid grid-cols-3 gap-4 mb-8 bg-slate-50 p-4 rounded-lg border">
          <div className="flex flex-col items-center p-2">
            ...
            <span className="font-bold text-lg">₹{scholarship.amount.toLocaleString("en-IN")}</span>
        ...
        <div className="space-y-6">
          ...
          {/* This part fixed the "Property eligibility does not exist" error */}
          {(scholarship as any).eligibility && (
            <section>
              <h3 className="text-xl font-semibold mb-3">Eligibility Criteria</h3>
              <ul className="space-y-2">
                {(scholarship as any).eligibility.map((item: string, i: number) => (
        ...
        <div className="mt-10 pt-6 border-t flex justify-end">
          <a href={(scholarship as any).applyLink} target="_blank" rel="noopener noreferrer">
